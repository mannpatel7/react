...
const Restaurantcard = ({ restaurant }) => {
    return (
        <Link to={`/restaurant/${restaurant.id}`} className="rest-card">
            <div className=" rounded-lg bg-gray-500 p-4 shadow-lg hover:shadow-xl transition-shadow duration-300">
                ...
                <div className="mt-4 p-2 flex flex-col gap-2">
                    ...
                    <p className="dish-cost">Cost for Two:<span className="font-bold">${restaurant.costForTwo/10}</span> </p>
                </div>
            </div>
        </Link>
    );
